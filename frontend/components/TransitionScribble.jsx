...
export default function TransitionScribble() {
    useEffect(() => {
        const logoAtomicClickable = document.querySelector('.logo-atomic');
        ...
        if (!logoAtomicClickable || !transitionScribblePath || !transitionScribbleSvg) return;
        ...
        const runScribbleAnimation = (e) => {
            ...
            if (!transitionLogo) {
                ...
                const svgClone = document.querySelector('.logo-atomic').cloneNode(true);
            ...
            drawTl.to(transitionLogo, {
                ...
                onStart: () => {
                    const transitionLogoGraphic = transitionLogo.querySelector('.logo-atomic');
                    if (transitionLogoGraphic) {
                        gsap.to(transitionLogoGraphic, { rotation: 5, duration: 0.15, repeat: -1, yoyo: true, ease: 'steps(1)', overwrite: 'auto' });
                    }
                }
            }, durIn * 0.5);
            ...
            drawTl.set(transitionLogo, {
                ...
                onComplete: () => {
                    const transitionLogoGraphic = transitionLogo.querySelector('.logo-atomic');
                    if (transitionLogoGraphic) {
                        gsap.killTweensOf(transitionLogoGraphic);
                        gsap.set(transitionLogoGraphic, { rotation: 0 });
                    }
                }
            }, durIn + (durOut * 0.48));
        };

        logoAtomicClickable.addEventListener('click', runScribbleAnimation);

        // Auto-run on load
        const timer = setTimeout(() => runScribbleAnimation(null), 100);

        return () => {
            logoAtomicClickable.removeEventListener('click', runScribbleAnimation);
            clearTimeout(timer);
        };
    }, []);
