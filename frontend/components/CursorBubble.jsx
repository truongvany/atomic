...
export default function CursorBubble() {
    useEffect(() => {
        ...
        const onMouseOver = (e) => {
            const targetSelector = '.footer-column h3, .footer-map-link span, .footer-email, .footer-whatsapp, .single-social, .logo-atomic, .nav-work-btn';
            const found = e.target.closest(targetSelector);

            if (found && !isHoveringClickable) {
                isHoveringClickable = true;
                if (found.matches('.logo-atomic')) cursorBubble.textContent = 'to home';
